---
order: 1280
title: Orchestration tab — The agent surface (SP3 of 3)
status: Done
priority: high
---
# Orchestration tab — The agent surface (SP3 of 3)

Two MCP tools, the live push into the open tab, the gavin-orchestrate
skill, and the Reorganize button.

Spec: `docs/superpowers/specs/2026-08-21-orchestration-tab-design.md`
Plan: `docs/superpowers/plans/2026-08-21-orchestration-agent-surface.md`

## Steps

- [x] GitDirtyPaths: capture helper, v2 path parser, request (+ tests)
- [x] Root-addressed orchestration requests and the OrchestrationChanged push (+ tests)
- [x] gavin_get_orchestration (+ tests)
- [x] gavin_set_orchestration (+ tests)
- [x] App: forward and apply the push
- [x] The gavin-orchestrate skill and multi-skill setup
- [x] The Reorganize with agent button
