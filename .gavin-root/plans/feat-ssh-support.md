...
- [ ] [Card runs on ssh workspaces: `compose_agent_prompt` and `setup_agent_integration` (MCP config) done on the host, where the agent and `gavin-mcp` run](./card-runs-on-ssh-workspaces-compose-agent-prompt-and-setup-agent-integration-mcp-config-done-on-the-host-where-the-agent-and-gavin-mcp-run.md)
- [ ] [Git tab and Files tab over ssh](./git-tab-and-files-tab-over-ssh.md)
