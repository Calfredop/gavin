...
- **Module boundary**: Svelte components never call `layout.ts`'s pure functions directly, and never call any *tree-mutating* `backend.ts` command (`createSession`, `killSession`, `getCurrentLayout`, `setLayout`) directly — they only call the exported actions from `layoutState.ts`. This keeps tree-mutation-plus-persistence atomic and in one place. `layoutState.ts` is the only module that imports both `layout.ts` and `backend.ts`. This rule does **not** extend to `backend.ts`'s two lifecycle-only wrappers, `signalFrontendReady` and `getBootstrapError` — they touch no tree state, so Task 7's top-level page imports and calls `signalFrontendReady` directly from `backend.ts` (see Task 7); this is intentional, not an exception to relitigate.
