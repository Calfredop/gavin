...
| O16 | **Starting a rail spawns its own page**, named after it, when the rail has none. An explicit binding is never overridden, and a page that still exists is reused — only an unbound or closed-page rail gets a fresh one. A failed creation is not a stall: the rail arms onto the Agents-page fallback. |
...
presetSingle, 1, name, checkout)` followed by `bindRail(railId, {
pageId })` — so the very first launch of the very first tick already
lands there. The page's own blank shell opens in the rail's checkout
(`worktreePath ?? root`, spelled as §4.3 spells it), so the page is the
rail's in the way that matters and not just by name. A rail
whose page was closed while it sat paused gets a new one on Resume for
the same reason. Creation failing is **not** a stall: the rail arms
anyway and its launches take §4.3's Agents-page fallback. A page is where
agents land, not a precondition for running them.
