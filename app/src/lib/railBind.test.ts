...
describe("the tab strip", () => {
  it("is the three bindings, in the order they take effect", () => {
    expect(RAIL_BIND_TABS.map((t) => t.id)).toEqual(["worktree", "branch", "page"]);
    expect(RAIL_BIND_TABS.map((t) => t.label)).toEqual(["Worktree", "Branch", "Page"]);
  });

  it("hands every tab a chip, so the strip can say the answers too", () => {
    const chips = railBindChips(UNBOUND, null);
    expect(chips.map((c) => c.tab)).toEqual(["worktree", "branch", "page"]);
  });
});
...
describe("what a chip says", () => {
  it("names the default, not a blank, when a binding is unset", () => {
    const [worktree, branch, page] = railBindChips(UNBOUND, null);
    expect(worktree.value).toBe("no worktree");
    expect(branch.value).toBe("any branch");
    expect(page.value).toBe("page at launch");
    // None of the three is an error: every one has a working default,
    // and colouring them as faults would put a warning on every new rail.
    expect([worktree.bound, branch.bound, page.bound]).toEqual([false, false, false]);
  });
...
describe("arrow keys across the strip", () => {
  it("wrap in both directions", () => {
    ...
    expect(railBindTabAfterKey("page", "ArrowRight")).toBe("worktree");
    expect(railBindTabAfterKey("worktree", "ArrowLeft")).toBe("page");
    expect(railBindTabAfterKey("branch", "ArrowLeft")).toBe("worktree");
  });

  it("jump to the ends", () => {
    expect(railBindTabAfterKey("branch", "Home")).toBe("worktree");
