...
describe("orderableHubViewIds / manageableHubViewIds", () => {
  ...
  // A stored order outlives the build that wrote it, so the checklist
  // keeps a place in the order even where it is never drawn -- but it is
  // a build artefact, not a section anyone chooses to keep, so it is not
  // a row in either settings panel.
  it("orders the dev-only checklist but never offers it as a row", () => {
    expect(orderableHubViewIds()).toContain("checklist");
    expect(manageableHubViewIds()).not.toContain("checklist");
  });
});
...
describe("tabStripHubViewIds with preferences", () => {
  ...
  it("drops the hidden ones", () => {
    const ids = tabStripHubViewIds("ws-1", false, true, { order: null, hidden: ["git", "prd"] });
  ...
  it("ignores a hidden set that would empty this workspace's strip", () => {
    expect(tabStripHubViewIds("ws-1", false, false, { order: null, hidden: ["kanban"] })).toEqual([
      "kanban",
    ]);
  });
  ...
  it("keeps counting the same list the strip draws", () => {
    ...
    const ids = tabStripHubViewIds("ws-1", false, true, prefs);
...
describe("resolveHubView", () => {
  ...
  it("falls back when the remembered tab has been hidden", () => {
    expect(
      resolveHubView(workspace({ hubView: "git" }), false, { order: null, hidden: ["git"] })
    ).toBe("home");
  });

  it("lands on the first tab of the human's own order", () => {
    expect(resolveHubView(workspace({}), false, { order: ["plans"], hidden: null })).toBe("plans");
  });
  ...
  it("keeps a button-reached view even though it is not in the strip", () => {
    expect(
      resolveHubView(workspace({ hubView: "settings" }), false, {
        order: null,
        hidden: ["git", "prd"],
      })
    ).toBe("settings");
